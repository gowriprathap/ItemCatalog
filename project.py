...
def showLogin():
    ...
    login_session['state'] = state #storing the state in a variable for later use
...
def gconnect():
 ...
 # If there was an error in the access token info, send a 500 internal server error to the client
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Current user already connected'), #The user is already connected
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    login_session['username'] = data['name'] # obtaining the user name from google log in
    login_session['picture'] = data['picture'] # obtaining the picture from google log in
    login_session['email'] = data['email'] # obtaining the email from google log in
    ...
    flash("Welcome to Musical Instruments Catalog, %s" % login_session['username'])
    #print "done!"
    return output
...
def getUserID(email): #Function to get user details through email from database
# Argument is email
#Return is user id
...
def gdisconnect(): # Function to disconnect from the google account
    ...
    if access_token is None:
        response = make_response(json.dumps('Current user not connected.'), #The user was not logged in
                                 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if result['status'] == '200':
        ...
        del login_session['username'] #deleting all the details and reseeting so that the user can log out
        del login_session['email']
        del login_session['picture']
        response = make_response(json.dumps('Successfully disconnected.'), 200)
        response.headers['Content-Type'] = 'application/json'
        flash("You have successfully logged out.") #Flashing a message when logged out
        return redirect(url_for('catalogFunction')) #returning to original page
        # return response
    else:

        response = make_response(
            json.dumps(
                'Failed to revoke token for given user.', #if the server is not able to retrieve the token to log the user out
                400))
        response.headers['Content-Type'] = 'application/json'
        return response
...
def catalogFunction():
    ...
    if 'username' not in login_session: #If the user is not logged in
        ...
        if category.count() == 0: #The user is logged in, and there are no categories
...
def newCategoryFunction():
    ...
    if 'username' not in login_session: #if the user is not logged in, then redirecting to the login page
        return redirect('/login')
    if request.method == 'POST':
        newCategory = Category(name = request.form['name'], user_id=login_session['user_id']) #Taking in information from the user
        session.add(newCategory)

        session.commit()
        flash('The new Category %s is successfully Created' % newCategory.name) #Flashing the message that the new category was created
        return redirect(url_for('catalogFunction', category=category)) #Redirecting to the catalog page
    else:
        return render_template('newCategory.html', category=category)

@app.route('/category/<int:category_id>/edit', methods=['GET', 'POST']) #To edit a category
def editCategoryFunction(category_id):
    ...
    # Check if user is logged. If not, redirect to login page
    if 'username' not in login_session: #If the user is not logged in, redirect to the login page
        return redirect('/login')
    if category2.user_id != login_session['user_id']: #If the user who created the category is not the one who is logged in now
        flash("Sorry, you not authorized to edit this category.") #That person is not authorized to edit
    ...
    if request.method == 'POST':
        ...
        session.commit() #Committing to the session to save changes
        flash("The category was successfully edited.") #Flashing a message to show that the cateogry was successfully edited
...
def deleteCategoryFunction(category_id):
  ...
  if 'username' not in login_session: #If the user is not logged in, redirect back to log in page
      return redirect('/login')
  if login_session['user_id'] != category2.user_id: #If the user who created the category is not the one who is logged in now
      flash ("Sorry, you cannot delete this category.")  #That person is not authorized to delete the category
      return redirect(url_for('catalogFunction'))
  if request.method == 'POST':
      ...
      return redirect(url_for('catalogFunction', m_instruments = category)) #Redirected to catalog page
  else:
...
def newItemFunction(category_id):
  ...
  if not hasattr(category2, 'id'):
      return "this category does not exist"
  if 'username' not in login_session: #If the user is not logged in, redirect to login page
      return redirect('/login')
  ...
  if request.method == 'POST':
      ...
      session.add(newItem)
      session.commit()
      flash('New %s Item is successfully created' % (newItem.name)) #Flash message that new item is successfully created
...
#Edit a menu item
@app.route('/category/<int:category_id>/items/<int:item_id>/edit', methods=['GET','POST']) #Editing the items in the categories
def editItemFunction(category_id, item_id):
    ...
    if request.method == 'POST': #Request form for making changes
        ...
        session.commit()
        flash(' Item Successfully Edited') #Flashing message that item was successfully edited
...
def deleteItemFunction(category_id, item_id):
    ...
    if 'username' not in login_session:
        return redirect('/login')
    # If the logged in user is not the creator of the tem, show an authorization message
    ...
    if request.method == 'POST':
        ...
        flash(' Item Successfully Deleted') #Flashing a message saying item was deleted
...
if __name__ == '__main__':
  ...
  app.run(host = '0.0.0.0', port = 5000) #To open the website on localhost:5000
